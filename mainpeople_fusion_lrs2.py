...
from dataset import LIPSMIXDATASET
from models import ModelBuilder,OurModelBUilder, activate
...
import wandb
wandb.init(project="Sound of Pixels for People Voice lips reading")
...
class NetWrapper(torch.nn.Module):
    ...
    def forward(self, batch_data, args):
        ...
        #print("FEat frame: ", feat_frames[0].shape)
        #print("Feat sound: ", feat_sound.shape)
        ...
        # 4. loss
        print("Pred mask  sahpe: ", pred_masks[0].shape)
        print("Gt mask  sahpe: ", gt_masks[0].shape)
...
class FusionNetWrapper(torch.nn.Module):
    def __init__(self, nets, crit):
        super(FusionNetWrapper, self).__init__()
        self.net_sound, self.net_frame_dilated, self.net_video, self.net_fusion = nets
        self.crit = crit
    ...
    def forward(self, batch_data, args):
        ...
        for n in range(N):
            ...
            fusion_data[n] = torch.cat((feat_frames[n], feat_sound), dim=1)
            # feat_frames[n] = activate(feat_frames[n], args.img_activation)
        ...
        pred_masks = [None for n in range(N)]
        for n in range(N):
            pred_masks[n] = self.net_fusion(fusion_data[n])
            pred_masks[n] = pred_masks[n].unsqueeze(1)
            pred_masks[n] = activate(pred_masks[n], args.output_activation)
...
def checkpoint(nets, history, epoch, args):
    ...
    torch.save(net_frame_dilated.state_dict(),
               '{}/frame_dilated_{}'.format(args.ckpt, suffix_latest))
    torch.save(net_video.state_dict(),
               '{}/video_{}'.format(args.ckpt, suffix_latest))

    torch.save(net_fusion.state_dict(),
               '{}/fusion_{}'.format(args.ckpt, suffix_latest))
    ...
    if cur_err < args.best_err:
        ...
        torch.save(net_frame_dilated.state_dict(),
               '{}/frame_dilated_{}'.format(args.ckpt, suffix_best))
        torch.save(net_video.state_dict(),
               '{}/video_{}'.format(args.ckpt, suffix_best))

        torch.save(net_fusion.state_dict(),
               '{}/fusion_{}'.format(args.ckpt, suffix_best))
...
def create_optimizer(nets, args):
    ...
    param_groups = [{'params': net_sound.parameters(), 'lr': args.lr_sound},
                    {'params': net_fusion.parameters(), 'lr': args.lr_synthesizer},
                    {'params': net_frame_dilated.features.parameters(), 'lr': args.lr_frame},
                    {'params': net_video.parameters(), 'lr': args.lr_frame}]
...
def main(args):
    # Network Builders
    builder = OurModelBUilder()
    net_sound = builder.build_sound()
    net_frame_dilated = builder.build_frame_dilated()
    net_video = builder.build_video()
    net_fusion = builder.build_fusion()

    nets = (net_sound, net_frame_dilated, net_video,  net_fusion)
    crit = builder.build_criterion(arch=args.loss)

    # Dataset and Loader
    scence_image_dir = "/home/vuthede/MyHelperModule/downloaddata/download_data_for_sound_of_pixel_paper/image_sence"
    ...
    # Wrap networks
    netWrapper = FusionNetWrapper(nets, crit)
    netWrapper = torch.nn.DataParallel(netWrapper, device_ids=[1])
